...
class Settings extends BaseApplication {
	...
	override public function foreground():Void {
		slider.value = Wasp.system.brightness - 1;
		draw();
	...
	override public function touch(event:TouchEvent):Void {
		switch (settingsIndex) {
			case 0: // Brightness
				slider.touch(event);
				Wasp.system.brightness = slider.value + 1;

			case 1: // Notification Level
				nfySlider.touch(event);
				Wasp.system.notifyLevel = nfySlider.value + 1;
			...
			case 4: // Units
				if (unitsToggle.touch(event)) {
					var index = (units.indexOf(Wasp.system.units) + 1) % units.length;
					Wasp.system.units = units[index];
				}
	...
	function draw():Void {
		...
		switch (settingsIndex) {
			...
			case 1: // Notification Level
				nfySlider.value = Wasp.system.notifyLevel - 1;
	...
	function update():Void {
		...
		switch (settingsIndex) {
			case 0: // Brightness
				var say = switch (Wasp.system.brightness) {
					case 3: "High";
					case 2: "Mid";
					case _: "Low";
				};
			...
			case 1: // Notification Level
				var say = switch (Wasp.system.notifyLevel) {
					case 3: "High";
					case 2: "Mid";
					case _: "Silent";
				};
			...
			case 4: // Units
				draw.string(Wasp.system.units, 0, 150, 240);
